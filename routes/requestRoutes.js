...
router.post("/paymentPoints", async (req, res) => {
    try {
        //load all the items into memory
        ...
        if (isValid) {
            ...
            try {
                //query to insert into collection request
                const insertRequest = await pool.query("INSERT INTO collectionRequests(sessionId, date, firstName, lastName, phoneNo, address, state, status, userId) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id", ["points", date, firstName, lastName, phoneNo, addressSQL, state, "pending", req.user.id]);
            
                //save the auto incremented id into memory
                const id = insertRequest.rows[0].id; 
            
                const updatePoints = await pool.query("UPDATE users SET points = points - 1000 WHERE id = $1", [req.user.id]);
